...
export const env = createEnv({
  ...
  server: {
    ...
    // MTG Import specific
    SCRYFALL_CACHE_DIR: z.string().optional().default("/tmp/scryfall-cache"),
    SCRYFALL_CACHE_MAX_AGE_HOURS: z.coerce.number().optional().default(6),

    // Import settings
    DEFAULT_CURRENCY: z.string().length(3).optional().default("USD"),
    IMPORT_BATCH_SIZE: z.coerce.number().optional().default(100),
    JOB_POLL_INTERVAL_MS: z.coerce.number().optional().default(5000),

    // Saleor channels (comma-separated IDs)
    DEFAULT_CHANNEL_SLUGS: z.string().optional().default("default-channel,singles-builder"),

    /*
     * Docker networking - URL aliases for APL normalization
     * Format: "alias1=canonical1,alias2=canonical2" (e.g., "localhost:8000=api:8000")
     * Default handles common Docker dev setups where browser uses localhost but containers use api
     */
  ...
  runtimeEnv: {
    ...
    // MTG Import specific
    SCRYFALL_CACHE_DIR: process.env.SCRYFALL_CACHE_DIR,
    SCRYFALL_CACHE_MAX_AGE_HOURS: process.env.SCRYFALL_CACHE_MAX_AGE_HOURS,
    DEFAULT_CURRENCY: process.env.DEFAULT_CURRENCY,
    IMPORT_BATCH_SIZE: process.env.IMPORT_BATCH_SIZE,
    JOB_POLL_INTERVAL_MS: process.env.JOB_POLL_INTERVAL_MS,
    DEFAULT_CHANNEL_SLUGS: process.env.DEFAULT_CHANNEL_SLUGS,
