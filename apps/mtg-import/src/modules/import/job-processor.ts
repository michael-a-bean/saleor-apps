...
export class JobProcessor {
  ...
  async processJob(job: ImportJob): Promise<ProcessResult> {
    ...
    try {
      ...
      // Phase 2: Attach images to products created without media
      logger.info("Phase 1 complete, starting Phase 2 (image attachment)", { jobId: job.id });
      const imageResult = await this.attachImages(job);
  ...
  /**
   * Phase 2: Attach images to products that were created without media.
   * Queries ImportedProduct by scope (setCode or global) with mediaAttached=false.
   * Calls productMediaCreate for each product, rate-limited.
   */
  private async attachImages(
    ...
    if ((job.type === "SET" || job.type === "BACKFILL") && job.setCode) {
      whereClause.setCode = job.setCode;
    }

    const products = await this.prisma.importedProduct.findMany({
      where: whereClause,
      select: {
        id: true,
    ...
    if (products.length === 0) {
      logger.info("Phase 2: No products need image attachment", { jobId: job.id });
      return { attached: 0, failed: 0 };
    }

    logger.info("Phase 2: Attaching images", {
      jobId: job.id,
    ...
    for (let i = 0; i < products.length; i += IMAGE_BATCH_SIZE) {
      ...
      if (this.abortController?.signal.aborted) {
        logger.info("Phase 2: Aborted", { jobId: job.id, attached, failed });
        break;
      }
      ...
      for (const product of batch) {
        // Derive image URL from scryfall_id stored in metadata
        // We use the Scryfall CDN URL pattern: https://cards.scryfall.io/large/front/{a}/{b}/{uuid}.jpg
        // But we need to fetch metadata from Saleor to get the stored scryfall_image_url
        // Since we stored it in metadata during Phase 1, query it
        const imageUrl = await this.getProductImageUrl(product.saleorProductId);

        if (!imageUrl) {
          // Product lacks scryfall_image_url metadata (likely imported before two-phase code).
          // Mark as attached to prevent repeated futile retries — these products either
          // already have images (old code path) or need manual repair via repairImages.
          logger.debug("Phase 2: No image URL in metadata, marking as handled", {
            productId: product.saleorProductId,
        ...
        const media = await this.saleor.createProductMedia(
      ...
      if (i + IMAGE_BATCH_SIZE < products.length && !this.abortController?.signal.aborted) {
        await new Promise((resolve) => setTimeout(resolve, BATCH_DELAY_MS));
      }

      // Log progress periodically
      if ((i + IMAGE_BATCH_SIZE) % 50 === 0 || i + IMAGE_BATCH_SIZE >= products.length) {
        logger.info("Phase 2: Progress", {
          jobId: job.id,
          processed: Math.min(i + IMAGE_BATCH_SIZE, products.length),
  ...
  /** Get the scryfall_image_url from a product's metadata */
  private async getProductImageUrl(saleorProductId: string): Promise<string | null> {
    try {
      const result = await this.saleor.getProductMetadata(saleorProductId);
      if (!result) return null;

      const imageEntry = result.find((m) => m.key === "scryfall_image_url");
      return imageEntry?.value ?? null;
    } catch {
      return null;
    }
  }
