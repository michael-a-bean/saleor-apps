...
const attachAppToken = middleware(async ({ ctx, next }) => {
  ...
  // APL normalizes URLs internally, but we also normalize here for logging consistency
 ...
 * Also fetches all installation IDs for the same Saleor API URL
 * to support cross-app data queries (e.g., inventory-ops cost events)
 */
const attachInstallationContext = middleware(async ({ ctx, next }) => {
  ...
  /*
   * Get all installation IDs for this Saleor API URL.
   * This allows POS to see cost layer events and customer credits from related apps
   */
  const allInstallations = await ctx.prisma.appInstallation.findMany({
    where: {
      saleorApiUrl: ctx.saleorApiUrl,
    },
    select: {
  ...
  const allInstallationIds = allInstallations.map((inst) => inst.id);
